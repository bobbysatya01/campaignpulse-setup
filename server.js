...
async function analyseCampaigns(campaigns) {
  ...
  for (let i = 0; i < campaigns.length; i++) {
    ...
    state.alerts.push({ campaignId: c.campaignId, name: name, type: alertType, time: timeStr, date: dateStr, budget: budget, acos: Math.round(acos * 10) / 10 });
    ...
    if (outOfBudget) {
      ...
      state.exhaustionLog.unshift({ date: now.toLocaleDateString('en-GB'), time: timeStr, campaign: name, budget: 'GBP' + budget.toFixed(2), acos: acos.toFixed(1) + '%', missed: 'GBP' + missed, added: 'Pending', action: 'Pending' });
      if (chatCount < maxChats) {
        const dashUrl = process.env.DASHBOARD_URL || 'https://campaignpulse-setup-production.up.railway.app';
        const msg = 'Campaign out of budget: ' + name + ' | Ran out at ' + timeStr + ' | Budget: GBP' + budget.toFixed(2) + ' | ACOS: ' + acos.toFixed(1) + '% | Est missed: GBP' + missed + ' | Approve at: ' + dashUrl;
        await sendGoogleChat(msg);
        chatCount++;
      }
    } else if (acosHigh && chatCount < maxChats) {
      const dashUrl = process.env.DASHBOARD_URL || 'https://campaignpulse-setup-production.up.railway.app';
      const msg = 'High ACOS alert: ' + name + ' | ACOS: ' + acos.toFixed(1) + '% | Spend: GBP' + spend.toFixed(2) + ' | Review at: ' + dashUrl;
      await sendGoogleChat(msg);
      chatCount++;
    } else if (budgetLow && chatCount < maxChats) {
      const dashUrl = process.env.DASHBOARD_URL || 'https://campaignpulse-setup-production.up.railway.app';
      const msg = 'Budget low: ' + name + ' | Remaining: GBP' + remaining.toFixed(2) + ' (' + remainingPct.toFixed(0) + '%) | ACOS: ' + acos.toFixed(1) + '% | Review at: ' + dashUrl;
      await sendGoogleChat(msg);
      chatCount++;
    }
  }
...
app.post('/api/campaigns/:id/budget', async function(req, res) {
  ...
  try {
    ...
    if (log) { log.added = '+GBP' + amount; log.action = 'Budget added'; }
    await sendGoogleChat('Budget approved for ' + campaign.name + ' | +GBP' + amount + ' added | New budget: GBP' + newBudget.toFixed(2));
    syncCampaigns();
