  ...
  try {
    ...
    try {
      const todayAlerts = await db.query(
        "SELECT campaign_id, campaign_name, problem_type, problem_detail, created_date as created_at FROM campaign_tasks WHERE task_source='alert' AND created_date=CURRENT_DATE"
      );
